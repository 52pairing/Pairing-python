...
class NegotiationService:
    ...
    def _build_prompt(self, request: ProposeRequest) -> str:
        ...
        return (
            ...
            "1-2) **첫 제안은 자기 측 희망값(클라희망/프리희망)에서 시작한다.** "
            "마지노선은 물러설 수 없는 방어선이지 시작점이 아니다. "
            "첫 발언에 자기 마지노선을 그대로 부르면 남은 협상 여지가 사라지고, "
            "대화 로그는 상대에게도 그대로 보이므로 내 선을 알려주는 것과 같다.\n"
            "1-3) **상대의 첫 제안을 그대로 수락하지 않는다.** "
            "접점이 있으면 최소 한 번은 자기 희망값 쪽으로 역제안(COUNTER)한 뒤 절충한다. "
            "즉시 수락하면 양보할 수 있었던 여지를 한 번에 다 내주고 상대 조건에서 끝난다 "
            "— 프리가 부른 시작일을 클라 대리인이 역제안 없이 받아 클라 마지노선 경계에서 "
            "합의된 적이 있다.\n"
            "1-3-1) 다만 이 규칙은 **마지노선 규칙보다 아래다.** 역제안할 여지가 없으면"
            "(접점이 한 점뿐이거나, 상대 제안이 이미 자기 희망값보다 유리하면) "
            "억지로 역제안하지 말고 그대로 수락한다. 여지가 없는데 버티는 것은 "
            "합의를 깨뜨릴 뿐이다.\n"
