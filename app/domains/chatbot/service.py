...
- MY_PROJECTS: 내가 등록했거나 참여 중인 프로젝트를 보러 가는 안내.
  프로젝트 목록·진행 상황·상태 확인, 매칭·추천·재추천이 여기에 해당한다.
  "내 프로젝트", "등록한 프로젝트", "받은 요청" 을 어디서 보냐는 질문은 전부 이것이다.
...
- INQUIRY_NEW: 페어링 서비스에 관한 질문인데 정책 범위 밖이라 답할 수 없을 때.
  화면으로 안내할 수 있으면 이것을 고르지 않는다. 위 화면 중 하나로 보낼 수 있는
  질문에 1:1 문의를 권하면, 사용자는 스스로 할 수 있는 일을 관리자에게 묻게 된다.
...
_SCREEN_GUIDE = """
- 이력서·포트폴리오 작성/수정: 마이페이지 > 내 이력서/포트폴리오 (프리랜서만)
...
역할 제한이 붙은 화면(위에 "(프리랜서만)", "(클라이언트만)" 이라고 적힌 것)을 안내할 때는
누가 쓸 수 있는 기능인지 답변에 반드시 한 문장으로 밝힌다.

묻는 사람의 역할을 모르기 때문이다. 쓸 수 없는 역할이 물었다면 그 사실을 알아야 하고,
쓸 수 있는 역할이 물었더라도 사실을 한 번 더 확인하는 것이라 해가 없다.
이 안내가 없으면, 프리랜서가 프로젝트 등록 방법만 배우고 왜 안 되는지는 모르는 채로 끝난다.
"""
